 ...
 */
export function createProcess(processPath, args = [], env = null) {
 ...
 */
function executeWithInput(processPath, args = [], inputs = [], opts: { env?: { DEBUG?: boolean }; timeout?: number; maxTimeout?: number } = {}) {
...
export const create = () => {
  const executable = path.resolve(__dirname, '..', 'src', 'vc.ts');
  const tsNode = path.resolve(__dirname, '..', 'node_modules', '.bin', 'ts-node');
  const fn = (...args) => executeWithInput(tsNode, ...[executable, ...args]);
  return {
    execute: fn
  };
}
